...
class ControllerPaymentMorposGateway extends Controller
{
    public function index()
    {
        ...
        $data = array(
            'confirm_url' => $this->url->link('payment/morpos_gateway/confirm', '', 'SSL'),
    ...
    public function confirm()
    {
        ...
        if (!$json) {
            ...
            $return_url = $this->url->link(
                'payment/morpos_gateway/callback',
                '',
                true
            ...
            $this->load->model('payment/morpos_conversation');
            $nextSeq = $this->model_payment_morpos_conversation->getNextAttemptSeq((int) $order_id);
            ...
            $this->model_payment_morpos_conversation->addAttempt(
                (int) $order_id,
                $nextSeq,
                $conversationId,
    ...
    public function callback()
    {
        ...
        if (!$order_id) {
            ...
            $this->response->redirect($this->generateUrlWithLanguage('payment/morpos_gateway/proxy') . '&data=' . urlencode($encrypted));
            return;
        }
    ...
    protected function handleCallback($order_id, $paid_status_id, $fail_status_id)
    {
        ...
        $conversationId = (string) $this->safeGet('ConversationId', $this->safeGet('conversationId', ''));
        $isValidConversation = $this->model_payment_morpos_conversation
