...
[![OpenCart Sürümü](https://img.shields.io/badge/OpenCart-2.0--2.2-blue.svg)](https://www.opencart.com/)
[![PHP Sürümü](https://img.shields.io/badge/PHP-5.3%2B-777bb4.svg)](https://php.net/)
[![Lisans](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)

**MorPOS for OpenCart**, OpenCart 2.0-2.2 mağazalarına **Morpara MorPOS** ödeme sistemini entegre eden güvenli ve kullanımı kolay bir ödeme ağ geçidi eklentisidir. Müşteriler, siparişlerini tamamlarken güvenli **Barındırılan Ödeme Sayfası (HPP)** akışı üzerinden yönlendirilir veya **Gömülü Ödeme Formu** kullanabilir.

## 📦 Diğer Versiyonlar

> **📌 Not**: Bu branch **OpenCart 2.0, 2.1 ve 2.2** sürümlerini desteklemektedir. Başka versiyonlar mı arıyorsunuz?
> - **OpenCart 2.3.x**: [2.3.x branch](https://github.com/morpara/morpos-opencart/tree/2.3.x) gidin
> - **OpenCart 3.x**: [3.x branch](https://github.com/morpara/morpos-opencart/tree/3.x) gidin
> - **OpenCart 4.x**: [4.x branch](https://github.com/morpara/morpos-opencart/tree/4.x) gidin

## ✨ Özellikler

- 🛒 **OpenCart Entegrasyonu**: OpenCart 2.0-2.2 için MorPOS'u ödeme yöntemi olarak sorunsuzca ekler
...
- `mysql` veya `mysqli` - Veritabanı işlemleri için gereklidir
...
2. **Dosyaları OpenCart'a Kopyalayın**
   ...
   cp -r upload/admin/controller/payment/morpos_gateway.php /path/to/opencart/admin/controller/payment/
   cp -r upload/admin/language/english/payment/morpos_gateway.php /path/to/opencart/admin/language/english/payment/
   cp -r upload/admin/language/turkish/payment/morpos_gateway.php /path/to/opencart/admin/language/turkish/payment/
   cp -r upload/admin/model/payment/morpos_gateway.php /path/to/opencart/admin/model/payment/
   cp -r upload/admin/view/template/payment/morpos_gateway.tpl /path/to/opencart/admin/view/template/payment/
   ...
   cp -r upload/catalog/controller/payment/morpos_gateway.php /path/to/opencart/catalog/controller/payment/
   cp -r upload/catalog/language/english/payment/morpos_gateway.php /path/to/opencart/catalog/language/english/payment/
   cp -r upload/catalog/language/turkish/payment/morpos_gateway.php /path/to/opencart/catalog/language/turkish/payment/
   cp -r upload/catalog/model/payment/morpos_gateway.php /path/to/opencart/catalog/model/payment/
   cp -r upload/catalog/model/payment/morpos_conversation.php /path/to/opencart/catalog/model/payment/
...
3. **Doğru İzinleri Ayarlayın**
   ...
   chmod 644 /path/to/opencart/admin/controller/payment/morpos_gateway.php
   chmod 644 /path/to/opencart/catalog/controller/payment/morpos_gateway.php
...
3. Başarılı bağlantı için yeşil onay işaretinin göründüğünü doğrulayın
...
1. **OpenCart 2.x:**
   ...
   // Veya hata ayıklama için hata görüntülemeyi etkinleştirin:
...
| Bileşen | Kontrol |
|---------|---------|
| **PHP Sürümü** | 5.3+ gerekli, 5.6/7.0 önerilen |
| **OpenCart Sürümü** | 2.0.0.0+ gerekli, 2.2.0.0 önerilen |
...
1. **Dil Dosyasını Kopyalayın:**
   ...
   cp upload/admin/language/english/payment/morpos_gateway.php \
      upload/admin/language/[dil-kodu]/payment/morpos_gateway.php
   
   # Katalog için (müşteriye yönelik)
   cp upload/catalog/language/english/payment/morpos_gateway.php \
      upload/catalog/language/[dil-kodu]/payment/morpos_gateway.php
   ```

2. **Metinleri Çevirin:**
   ```php
   <?php
   // Örnek: Almanca (german)
...
2. **Yerel Ortamı Kurun**
   - OpenCart 2.0-2.2'yi yerel olarak yükleyin
...
3. **Değişiklik Yapın**
   ...
   - OpenCart 2.0, 2.1 ve 2.2 ile test edin
...
- OpenCart 2.0-2.2 ile uyumluluğu koruyun
- Fonksiyonlar ve sınıflar için PHPDoc yorumları ekleyin
- Anlamlı commit mesajları yazın
- OpenCart 2.x dizin yapısını takip edin (ödeme dosyaları `controller/payment/` ve `language/*/payment/` içinde)
- `.tpl` şablon dosyalarını kullanın (`.twig` değil)
- Dil dosyası yüklemek için `$this->language->load()` kullanın
...
5. **Uyumluluk**
   - OpenCart 2.0.0.0, 2.1.0.0, 2.2.0.0
